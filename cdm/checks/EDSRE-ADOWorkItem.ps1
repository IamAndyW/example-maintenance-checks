...
$script:wiUserStories = Find-ADOWorkItemsByQuery -baseURL $adoConfiguration.baseURL -systemAccessToken $adoConfiguration.token -wiQuery $wiUserStoryQuery
...
if ($wiUserStories.workItems.Count -eq 0) {
    ...
    $script:newWI = New-ADOWorkItem -baseURL $adoConfiguration.baseURL -systemAccessToken $adoConfiguration.token -wiTitle $wiTitle -wiDescription $wiDescription
    ...
    $script:wiThemes = Find-ADOWorkItemsByQuery -baseURL $adoConfiguration.baseURL -systemAccessToken $adoConfiguration.token -wiQuery $wiThemeQuery

    foreach ($wiThemeChildId in $wiThemes.workItemRelations.target.id) {
        $wiThemeChild = Get-ADOWorkItemById -baseURL $adoConfiguration.baseURL -systemAccessToken $adoConfiguration.token -wiId $wiThemeChildId
        ...
        if ($wiThemeChild.fields.'System.Title' -eq $edSREClient) {
            ...
            $wiEpicChild = Get-ADOWorkItemById -baseURL $adoConfiguration.baseURL -systemAccessToken $adoConfiguration.token -wiId (Split-Path -Path $wiEpicChildLink.url -Leaf)
                
            if ($wiEpicChild.fields.'System.Title' -eq "Maintenance Planning") {
                    Link-ADOWorkItemToParent -baseURL $adoConfiguration.baseURL -systemAccessToken $adoConfiguration.token -wiId $newWI.Id -wiParentId $wiEpicChild.id
                }
            }  
        }
    }

    Write-Host ("Work item link: {0}" -f $newWI)
